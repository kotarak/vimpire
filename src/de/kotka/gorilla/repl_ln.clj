...
;;  in the file LICENSE.txt at the root of this distribution. By using this
...
;;
;;  Modified by Meikel Brandmeyer <mb at _remove_ kotka de>
;;  to accept \newline and \return as whitespace and ASCII nul
;;  as \newline.
;;  11th December 2008

(ns de.kotka.gorilla.repl-ln
  (:gen-class)
...
(defn- read-hook
  ...
  (let [{:keys [prompt flush read]} *private*]
    (loop [c (find-readable-this-line *in*)]
      (case c
       :eos eof
       :eol (recur (find-readable-this-line *in*))
       :eoi
       (do
         (prompt)
         (flush)
...
(defn repl
  ...
  (let [{:keys [init prompt flush read eval print caught in out err
    ...
    (try
     ...
     ;; unread null to enable first prompt
     (.unread *in* 0)
     (clojure.main/repl
      :init init
      :prompt #()
